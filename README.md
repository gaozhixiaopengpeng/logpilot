...
- 两个 Key 都配置或都未配置，且未显式设置 `AI_PROVIDER`：运行时会给出提示，请在 `.env` 中明确写上 `AI_PROVIDER=openai` 或 `AI_PROVIDER=deepseek`
...
```bash
cp .env.example .env
# 填写 OPEN_AI_API_KEY / DEEPSEEK_API_KEY（可选 AI_PROVIDER），不再兼容旧的 WORKLOG_* / LOGPILOT_* 变量
```

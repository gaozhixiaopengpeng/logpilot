...
**语言说明：** 默认仓库主文档为英文；本页为中文说明。[English](README.md)
...
- **终端界面**（帮助、报错、进度提示，以及打印在正文前的**报表标题行**）：根据系统语言环境（`LANG` / `LC_*` / `Intl`）在**英文与简体中文**间选择；中文环境为中文，否则为英文。通常**不必**手动设置这些变量，由操作系统或 shell 注入；CLI 读取它们以兼容常见 locale 约定。
- **`day` / `week` / `month` 的 `--lang`**：控制**模型生成的报表正文语言**；**省略时与终端界面语言一致**（英文界面→英文正文，中文界面→中文正文）。显式传入可覆盖（例如中文界面下仍可用 `--lang en`）。不改变上述界面文案与标题行（标题行仍随 UI）。
